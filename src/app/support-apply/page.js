...
export default function SupportApplyPage() {
  ...
  const handleNextToStep2 = () => {
    if (!fullName || !birthDate || !nationality || !residence || !email || !telegram || !joinReason || !freeTime) {
      setError("من فضلك قم بتعبئة جميع الحقول الإلزامية في هذا القسم.");
      return;
    }
    setError("");
    setCurrentStep(2);
  ...
  const SkillRating = ({ question, value, onChange }) => {
    return (
  ...
  return (
    <main className="feedback-page">
      <div className="feedback-card" style={{ transition: "all 0.3s ease" }}>
        ...
        {submitted ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {currentStep === 1 && (
                <div className="step-container">
                  ...
                  <div className="feedback-group">
                    ...
                    <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="أدخل اسمك الثلاثي" className="feedback-textarea" style={{ height: "auto", padding: "0.85rem 1rem" }} />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">تاريخ الميلاد *</label>
                    <input type="date" max="2010-12-31" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} className="feedback-textarea" style={{ height: "auto", padding: "0.85rem 1rem", direction: "rtl" }} />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">الجنسية *</label>
                    <input type="text" value={nationality} onChange={(e) => setNationality(e.target.value)} placeholder="مثال: سعودي" className="feedback-textarea" style={{ height: "auto", padding: "0.85rem 1rem" }} />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">مكان السكن الحالي *</label>
                    <input type="text" value={residence} onChange={(e) => setResidence(e.target.value)} placeholder="المدينة والدولة" className="feedback-textarea" style={{ height: "auto", padding: "0.85rem 1rem" }} />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">البريد الإلكتروني *</label>
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="example@mail.com" className="feedback-textarea" style={{ height: "auto", padding: "0.85rem 1rem", direction: "ltr", textAlign: "right" }} />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">حساب تيليجرام (Username) *</label>
                    <input type="text" value={telegram} onChange={(e) => setTelegram(e.target.value)} placeholder="@username" className="feedback-textarea" style={{ height: "auto", padding: "0.85rem 1rem", direction: "ltr", textAlign: "right" }} />
                  </div>
                  ...
                  <div className="feedback-group">
                    ...
                    <textarea rows={3} value={joinReason} onChange={(e) => setJoinReason(e.target.value)} placeholder="اكتب سبب انضمامك وشغفك هنا..." className="feedback-textarea" />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">أوقات التفرغ المناسبة لك للعمل التطوعي *</label>
                    <input type="text" value={freeTime} onChange={(e) => setFreeTime(e.target.value)} placeholder="مثال: الفترة المسائية، أيام الويكند، إلخ" className="feedback-textarea" style={{ height: "auto", padding: "0.85rem 1rem" }} />
                  </div>
              ...
              {currentStep === 2 && (
                <div className="step-container">
                  <h2 className="feedback-title" style={{ textAlign: "right" }}>قسم المهارات </h2>
                  <p className="feedback-subtitle" style={{ textAlign: "right" }}>يرجى تقييم مهاراتك الذاتية بكل شفافية.</p>

                  <SkillRating question="ماهي خبرتك بالتواصل باللغة العربية الفصحى؟" value={arabicSkill} onChange={setArabicSkill} />
                  <SkillRating question="كيف تقيِّم مهاراتك في التواصل الكتابي مع المستفيدين؟" value={writtenCommSkill} onChange={setWrittenCommSkill} />
                  <SkillRating question="كيف تقيِّم قدرتك على شرح المعلومات بطريقة بسيطة وواضحة؟" value={explainingSkill} onChange={setExplainingSkill} />
                  ...
                  <div style={{ display: "flex", gap: "1rem", marginTop: "2rem" }}>
                    ...
                    <button type="button" onClick={handleNextToStep3} className="feedback-submit">اختبار ردود الفعل ←</button>
                  </div>
                </div>
              )}
              ...
              {currentStep === 3 && (
                <div className="step-container">
                  ...
                  <div className="feedback-group">
                    ...
                    <textarea rows={3} value={scenario1} onChange={(e) => setScenario1(e.target.value)} placeholder="اكتب تصرفك هنا..." className="feedback-textarea" />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">2. قدمت معلومة للمستفيد، ثم اكتشفت أنها خاطئة. ماذا ستفعل؟ *</label>
                    <textarea rows={3} value={scenario2} onChange={(e) => setScenario2(e.target.value)} placeholder="اكتب تصرفك هنا..." className="feedback-textarea" />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">3. لاحظت أن أحد أعضاء الفريق يرد على المستفيدين بطريقة غير مناسبة، ماذا ستفعل؟ *</label>
                    <textarea rows={3} value={scenario3} onChange={(e) => setScenario3(e.target.value)} placeholder="اكتب تصرفك هنا..." className="feedback-textarea" />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">4. طلب منك شخص معلومات أو بيانات خاصة بأحد المستفيدين، ماذا سيكون تصرفك؟ *</label>
                    <textarea rows={3} value={scenario4} onChange={(e) => setScenario4(e.target.value)} placeholder="اكتب تصرفك هنا..." className="feedback-textarea" />
                  </div>

                  <div className="feedback-group">
                    <label className="feedback-legend">5. إذا قُبلت في الفريق ثم شعرت بعد شهر أنك لا تستطيع الالتزام، ماذا ستفعل؟ *</label>
                    <textarea rows={3} value={scenario5} onChange={(e) => setScenario5(e.target.value)} placeholder="اكتب تصرفك هنا..." className="feedback-textarea" />
                  </div>
