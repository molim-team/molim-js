...
const loadLocal = (uid) => {
  if (typeof window === "undefined") return []; // حماية من أخطاء السيرفر
  try {
...
export function FavoritesProvider({ children }) {
  ...
  const toggleFav = async (scholarshipId) => {
    if (!user) {
      alert("عذراً، يجب عليك تسجيل الدخول للاستفادة من خاصية المفضلة.");
      // اختياري: يمكنك هنا التوجيه لصفحة تسجيل الدخول إذا أردت
